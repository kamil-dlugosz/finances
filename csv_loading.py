...
from config import CSV_LOADING


def _load_single_csv(csv_path: Path) -> pd.DataFrame:
    df = pd.read_csv(csv_path, sep=";", decimal=",", dtype=str)

    for column in CSV_LOADING.DATE_TYPE_COLUMNS:
        df[column] = pd.to_datetime(df[column], format="%d.%m.%Y")

    for column in CSV_LOADING.FLOAT_TYPE_COLUMNS:
        df[column] = (
            df[column]
...
def load_all_csv_files(input_dir: Path) -> pd.DataFrame:
    ...
    combined_df = combined_df.rename(columns=CSV_LOADING.COLUMN_NAME_MAPPING)
